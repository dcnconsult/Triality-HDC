...
def main():
    ...
    print(f"R2={R2:.3f}, R3={R3:.3f}")
    # Koopman via lagged snapshots
    X = z[:, :-1].real
    Y = z[:, 1:  ].real
    evals, evecs, K = edmd(X, Y, dict_fn=lambda S: np.vstack([S, S**2]))
    # find dominant magnitude eigenvalue < 1
    idx = np.argsort(-np.abs(evals))
    dom = evals[idx[0]]
    print(f"Leading Koopman eigenvalue magnitude: {np.abs(dom):.4f}")
    plt.figure()
    plt.hist(np.abs(evals), bins=40)
    plt.title('Koopman eigenvalue magnitudes')
