...
def run_case(wb, wc, tag):
    ...
    # real signals for STFT
    x, y, z = a.real, b.real, c.real
    B, bic = bicoherence(x,y,z, nperseg=512, noverlap=256)
    vmax = np.nanmax(bic)
    print(f"{tag}: max bicoherence = {vmax:.3f}")
    plt.figure()
    plt.imshow(bic.T, origin='lower', aspect='auto')
    plt.title(f'Bicoherence map: {tag}')
    plt.xlabel('f1 index'); plt.ylabel('f2 index')
...
def main():
    ...
    # rational-nearby case (e.g., 3/2 and 5/3 approximants)
