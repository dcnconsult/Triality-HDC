...
def sl_triad_rhs(t, y, N, edges3, alpha=0.2, J=0.1, noise=0.0, rng=None):
    z = y[:N] + 1j*y[N:]; dz = (alpha - np.abs(z)**2)*z
    for i,j,k in edges3:
        dz[i] += J*z[j]*z[k]; dz[j] += J*z[i]*z[k]; dz[k] += J*z[i]*z[j]
...
def simulate_sl(N=200, m_edges=300, alpha=0.2, J=0.08, T=200, max_step=0.2, seed=0):
    ...
    y0c = 0.1*(rng.standard_normal(N)+1j*rng.standard_normal(N)); y0 = np.concatenate([y0c.real, y0c.imag])
    sol = solve_ivp(lambda t,y: sl_triad_rhs(t,y,N,edges3,alpha,J,0.0,rng), (0,T), y0, max_step=max_step)
    z = sol.y[:N] + 1j*sol.y[N:]; return sol.t, z, edges3
def order_params(z):
    phi = np.angle(z); R2 = np.abs(np.mean(np.exp(1j*phi)))
    N = z.shape[0]; rng = np.random.default_rng(0)
    triads = [tuple(rng.choice(N,3,replace=False)) for _ in range(min(2000, N))]
    vals = [np.exp(1j*(phi[i]+phi[j]+phi[k])) for i,j,k in triads]
    R3 = float(np.abs(np.mean(vals))); return float(R2), R3
