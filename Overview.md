...
$$
\mathcal{T}(s):=|\mathrm{II}_s|_{g_s}.
$$
...
$$
\dot{z}_i=(\alpha - |z_i|^2)z_i + \sum_{(i,j,k)\in \mathcal{E}_3} J_{ijk} z_j z_k + \eta_i(t).
$$
Order parameters: pairwise $R_2=\left|\frac{1}{N}\sum_i e^{i\phi_i}\right|$; triadic
$$
R_3=\left|\frac{1}{|\mathcal{E}_3|}\sum_{(i,j,k)} e^{i(\phi_i+\phi_j+\phi_k)}\right|.
$$
...
* **HDC:**
  ...
  * $\mathbf{F}_{k+1} = P_\varphi \mathbf{F}_k$ (fixed permutation).
